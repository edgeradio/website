...
$gnid = $_GET['id'];
 ...
 ?>
 <?php
 $infoprogprogprogresult = mysql_query("SELECT *, REPLACE(end_time, '00:00:00', '24:00:00'), DATE_FORMAT(start_time, '%H') as restream_time, DATE_FORMAT(start_time, '%l:%i %p') as start_time, DATE_FORMAT(end_time, '%l:%i %p') as end_time, DATE_FORMAT(r_start_time, '%l:%i %p') as r_start_time, DATE_FORMAT(r_end_time, '%l:%i %p') as r_end_time FROM program_info WHERE seotitle='$nameid' LIMIT 1") or die(mysql_error());
  ...
  <div id="main_container">
    ...
    <link rel="image_src" href="<?php echo $infoprogprogprog['sml_img']; ?>" />
      ...
      <div style="padding: 10px; -moz-border-top-left-radius:15px; -webkit-border-top-left-radius:15px; -moz-border-top-right-radius:15px; -webkit-border-top-right-radius:15px; height: 180px; background-image: url(<?php if($infoprogprogprog['xl_img']) { echo $infoprogprogprog['xl_img']; } else { echo'http://www.edgeradio.org.au/images/edge-blank-page.png'; } ?>);">
        ...
        <img style="margin-left: 5px; float: right; margin-top: 8px; margin-left: 5px;" title="Re-Stream Previous Episodes" src="http://edgeradio.org.au/images/restream.png">
        <?php } ?>
       ...
       <?php } ?>
        ...
        <div style="width:770px; margin: 0 auto; text-align:left; height:100%; display:block; float: left;">
        
        <div class="indentmenu" style="width: 770px;">
        <ul>
        <li><a href="http://www.edgeradio.org.au/program/<?php echo $gnid; ?>/">Program Home</a></li>
        <li><a href="http://www.edgeradio.org.au/program/<?php echo $gnid; ?>/playlists/">Playlists</a></li>
        <?php if($infoprogprogprog['restream_enabled'] == 'true') { ?><li><a href="http://www.edgeradio.org.au/program/<?php echo $gnid; ?>/restream/">Re-Stream</a></li><?php } ?>
        <li><a href="http://www.edgeradio.org.au/program/<?php echo $gnid; ?>/blog/">Blog</a></li>
		<li><a href="http://www.edgeradio.org.au/program/<?php echo $gnid; ?>/comments/">Comments</a></li>
      </ul>
      </div>
...
$pagination = "";
	...
	<?=$pagination?>
		...
		?>
		<div style="background-color: #F0F0F0; -moz-border-bottom-left-radius:15px; -webkit-border-bottom-left-radius:15px; -moz-border-bottom-right-radius:15px; -webkit-border-bottom-right-radius:15px;">
		<div style="font-size: 48px; padding: 15px 10px 15px 10px;" class="title">PLAYLISTS.</div>
          </div>
          <br>
		<div class="rounded">
		...
		?>
		<div style="background-color: #F0F0F0; -moz-border-bottom-left-radius:15px; -webkit-border-bottom-left-radius:15px; -moz-border-bottom-right-radius:15px; -webkit-border-bottom-right-radius:15px;">
		<div style="font-size: 48px; padding: 15px 10px 15px 10px;" class="title">RE-STREAM.</div>
          </div>
          <br>
...
$urlr = str_replace(" ", "%20", "http://mymyrecords.com/7edg".$day." EDGELOG ".$time."-00-00.mp3");
...
$timerr = $time+2;
     ...
     $urla = "".$day." EDGELOG ".$time."-00-00";
     $urlhrs = "".$day." EDGELOG ".$timer."-00-00";
     $urlhrss = "".$day." EDGELOG ".$timerr."-00-00";
...
echo'
     ...
     $urllw = str_replace(" ", "%20", "http://mymyrecords.com/7edg".date('Y-m-d', strtotime($day.'- 1 week'))." EDGELOG ".$time."-00-00.mp3");
     
     if(checkRemoteFile($urllw)) {
     
          $timer = $time+1;
     
     $urla = "".date('Y-m-d', strtotime($day.'- 1 week'))." EDGELOG ".$time."-00-00";
     $urlhrs = "".date('Y-m-d', strtotime($day.'- 1 week'))." EDGELOG ".$timer."-00-00";
...
echo'
     ...
     $urllw2 = str_replace(" ", "%20", "http://mymyrecords.com/7edg".date('Y-m-d', strtotime($day.'- 2 weeks'))." EDGELOG ".$time."-00-00.mp3");
     
     if(checkRemoteFile($urllw2)) {
     
          $timer = $time+1;
     
     $urla = "".date('Y-m-d', strtotime($day.'- 2 weeks'))." EDGELOG ".$time."-00-00";
  $urlhrs = "".date('Y-m-d', strtotime($day.'- 2 weeks'))." EDGELOG ".$timer."-00-00";
