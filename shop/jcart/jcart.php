...
class Jcart {
	...
	*/
	public function display_cart() {
		...
		echo tab(1) . "<form method='post' name='theForm' action='$checkout'>\n";
		echo tab(2) . "<fieldset>\n";
		...
		if($this->itemCount > 0) {
			...
			foreach($this->get_contents() as $item)	{
				...
				echo tab(7) . "<input id='jcartItemQty-{$item['id']}' name='jcartItemQty[]' style='width: 20px;' type='text' value='{$item['qty']}' />\n";
				echo tab(6) . "</td>\n";
				echo tab(6) . "<td class='jcart-item-name'>\n";

				if ($item['url']) {
					echo tab(7) . "{$item['name']} - Size: {$item['url']}\n";
				}
				else {
		...
		if ($isCheckout === true) {
			...
			echo tab(3) . "
<br>
			<p>Please Note: We are only able to ship to Australia. If you are after any merch outside of Australia, please <a href='mailto:admin@edgeradio.org.au'>email us</a>.</p>
<br>

<p>Payment is sent using PayPal secure transations, pay via Credit Card or your PayPal account. You are able to enter your shipping address and full details using PayPal.</p>
<br>
			
			<input type='$inputType' $src id='jcart-paypal-checkout' name='jcartPaypalCheckout' value='{$config['text']['checkoutPaypal']}' $disablePaypalCheckout />\n";
		}
