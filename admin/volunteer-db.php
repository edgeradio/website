...
?>
 ...
 mysql_select_db('edge_newsletter'); 
    ...
    <div id="two_column">
        ...
        <ul>
        <li><a href="volunteer-db.php?do=add">Add New Volunteer</a></li>
      </ul>
      </div>
<br />
...
$inforesult = mysql_query("SELECT * FROM volunteers WHERE email = '$email'");
$noof = mysql_num_rows($inforesult);
echo mysql_error();

if($noof == 0) {

 $query1 = "INSERT INTO volunteers (email) VALUES ('$email')";
...
?>
<form enctype="multipart/form-data" action="volunteer-db.php?do=add" method="post">
<h1 class="greyheader">Add New Volunteer</h1>
...
$email = $_POST['email'];
 ...
 $query1 = "UPDATE volunteers SET email='$email' WHERE id = '$id'";
 $result1 = mysql_query($query1);
 echo mysql_error();
 if($query1) {
  
   echo "<p><strong>Success!</strong></p><p><a href='volunteer-db.php'>Return To Volunteer Manager</a></p>";
 ...
 echo "</p><br>";
  ...
  $id = $_GET['id'];
$inforesult = mysql_query("SELECT * FROM volunteers WHERE id = '$id'");
while ($info = mysql_fetch_array($inforesult)) {
?>
<form enctype="multipart/form-data" action="volunteer-db.php?do=edit&id=<?php print $id ?>" method="post">
...
<input type="hidden" name="submitted" value="TRUE" /><input type="hidden" name="id" value="<?php echo $id; ?>" />
   ...
   $bid = $_GET['id'];
   $deletequery = mysql_query("DELETE FROM volunteers WHERE id='$bid'");
   ...
   echo "<meta http-equiv='refresh' content='=2;volunteer-db.php' />";
   } else {
    echo'
       <h1 class="greyheader">DELETED!</h1>
   ...
   echo "<meta http-equiv='refresh' content='=2;volunteer-db.php' />";
    ...
    <h1 class="greyheader">VOLUNTEER EMAIL MANAGER</h1>
...
$id = $_GET['id'];
  ...
  $myquery = "SELECT COUNT(*) as num FROM volunteers ORDER BY id DESC";
    $adjacents = 2;
	$total_pages = mysql_fetch_array(mysql_query($myquery));
	$total_pages = $total_pages[num];
	
	
	$targetpage = "volunteer-db.php";
	$limit = 15; 
	$page = $_GET['p'];
	if($page) 
		$start = ($page - 1) * $limit; 
...
$inforesult = mysql_query("SELECT * FROM volunteers ORDER BY email ASC LIMIT $start, $limit",$db);

$totalres = mysql_result(mysql_query("SELECT COUNT(*) as tot FROM volunteers ORDER BY email ASC"),0);	
...
<td><?php echo $info['email']; ?></td>
	<td><a href="volunteer-db.php?do=edit&id=<?php echo $info['id']; ?>" style="color: #FF9900; font-weight: bold; text-decoration: none;">Edit</a><br><a href="volunteer-db.php?do=delete&id=<?php echo $info['id']; ?>" style="color: #FF0000; font-weight: bold; text-decoration: none;">Delete</a></td>
</tr>
<?php
$count++;
}
