...
$restream = '<br><img title="Re-streaming Available" src="http://www.edgeradio.org.au/images/restream-mini.png">';
} else {
$restream = '';
}

  $event = new Event('<a style="color: #000000;" href="http://www.edgeradio.org.au/program/'.str_replace(" ","-",$info['seotitle']).'/">'.$title.'</a>', ''.$day_time.'', ''.$start_time.'', ''.$end_time.'', ''.stripslashes($info['summary']).$restream.'');
   $sm->addEvent($event);
    ...
    if(!$info['r_day_time'] == '') {
      ...
      if($info['r_day_time'] == 'Monday') {
   $r_day_time = 1;
   } elseif($info['r_day_time'] == 'Tuesday') {
   $r_day_time = 2;
   } elseif($info['r_day_time'] == 'Wednesday') {
   $r_day_time = 3;
   } elseif($info['r_day_time'] == 'Thursday') {
   $r_day_time = 4;
   } elseif($info['r_day_time'] == 'Friday') {
   $r_day_time = 5;
   } elseif($info['r_day_time'] == 'Saturday') {
   $r_day_time = 6;
   } elseif($info['r_day_time'] == 'Sunday') {
   $r_day_time = 7;
   } 
   
   
   if($info['r_end_time'] == '00:00') {
 $r_end_time = '24:00';
 } else {
  $r_end_time = $info['r_end_time'];
  }

$r_start_time = $info['r_start_time'];
...
$restream = '';
  ...
  $event = new Event('<a style="color: #000000;" href="http://www.edgeradio.org.au/program/'.str_replace(" ","-",$info['seotitle']).'/">'.$title.'</a>', ''.$r_day_time.'', ''.$r_start_time.'', ''.$r_end_time.'', ''.stripslashes($info['summary']).$restream.'');
   $sm->addEvent($event);
