...
<?php include '../../inc/sidebar.php'; ?>
 ...
 <div class="roundednew">
  <h1 class="title-head-right">New Tracks On Edge</h1>
  
  <div class="top_exp">
  <h1>Tune In!</h1>
  <p>Every day of the week between 9am and 10am you can hear an hour of the latest tracks added to Edge Radio's illustrious catalogue of music.</p>
  </div>
...
mysql_select_db("edge_music",$db);
   ...
   $myquery = "SELECT COUNT(*) as num FROM music WHERE public = 'true' AND type='track' ORDER BY date_added DESC";
...
$inforesult = mysql_query("SELECT * FROM music WHERE public = 'true' AND type='track' ORDER BY id DESC, date_added DESC LIMIT $start, $limit",$db);
$totalres = mysql_result(mysql_query("SELECT COUNT(*) as tot FROM music WHERE public = 'true' AND type='track' ORDER BY date_added DESC"),0);	
...
<tr>
	<td style="width: 100px;"><div style="background-image: url(http://www.edgeradio.org.au/images/unknown.jpg); width: 100px; height: 100px;"><img style="width:100px;height:100px;" src="'.$info['image'].'"></div><p></td>
	<td>
	<span class="title">'.stripslashes($info['artist']).' - '.stripslashes($info['title']).' <br> '; if($info['album']) { echo'<span style="color: #333333;">'.stripslashes($info['album']).'</span>'; } echo'</span>'; if($info['tasmanian']) { echo' <img title="Tasmanian Artist" src="http://www.edgeradio.org.au/images/tas_icon.png">'; } echo'<br>
	<span>| <b>From:</b> '.stripslashes($info['country']).' | <b>Genre:</b> '.stripslashes($info['genre']).' | <b>Label:</b> '.stripslashes($info['label']).' |</span><br><br>
	';
	if($info['soundcloud']) {
	 echo'
	 
	 <iframe class="iframe" src="https://w.soundcloud.com/player?url='.$info['soundcloud'].'&auto_play=false&show_artwork=false&show_comments=false&sharing=true" width="100%" height="166" scrolling="no" frameborder="no">...</iframe>
	';
	}
	echo'
	</td>
